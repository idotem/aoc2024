...
func getCombination(i, combsLen, numOfComb int64) []string {
	...
	combsInBinary := intToBinary(i, numOfComb)
...
func intToBinary(n, maxNumber int64) []string {
	maxBits := len(strconv.FormatInt(int64(maxNumber), len(OPERANDS)))

	binary := strconv.FormatInt(int64(n), len(OPERANDS))

	paddedBinary := fmt.Sprintf("%0*s", maxBits, binary)
